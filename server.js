...
function api(req, res, url) {
  ...
  if (req.method === 'POST' && url === '/api/reservations') return parseBody(req).then(body => { const u = user(req); if (!u || u.role !== 'student') return send(res, 401, { error: '请先登录学生账号' }); const db = read(); const required = ['roomId', 'seatId', 'date', 'start', 'end']; if (required.some(k => !body[k])) return send(res, 400, { error: '预约信息不完整' }); if (body.start >= body.end) return send(res, 400, { error: '结束时间必须晚于开始时间' }); const seat = db.seats.find(x => x.id == body.seatId); const room = db.rooms.find(x => x.id == body.roomId); if (!seat || !room || seat.roomId !== Number(body.roomId)) return send(res, 400, { error: '座位或阅览室不存在' }); if (room.status !== '开放') return send(res, 409, { error: '该阅览室当前暂停开放' }); if (seat.status !== '可预约') return send(res, 409, { error: '该座位当前不可预约' }); const overlap = db.reservations.find(x => x.seatId == body.seatId && x.date === body.date && ['待签到','使用中'].includes(x.status) && body.start < x.end && body.end > x.start); if (overlap) return send(res, 409, { error: '该时间段已经被预约' }); const record = { id: Date.now(), userId: u.id, userName: u.name, roomId: Number(body.roomId), seatId: Number(body.seatId), seatNo: body.seatNo, date: body.date, start: body.start, end: body.end, preferences: body.preferences || [], status: '待签到', createdAt: new Date().toISOString() }; db.reservations.unshift(record); save(db); send(res, 201, record); });
  if (req.method === 'PATCH' && url.startsWith('/api/reservations/')) return parseBody(req).then(body => { const db = read(); const u = user(req); const record = db.reservations.find(x => x.id === idFrom(url)); if (!record) return send(res, 404, { error: '预约不存在' }); if (!u || (u.role !== 'admin' && record.userId !== u.id)) return send(res, 403, { error: '无权操作该预约' }); if (body.action === 'cancel' && ['待签到','使用中'].includes(record.status)) record.status = '已取消'; if (body.action === 'checkin' && record.status === '待签到') { record.status = '使用中'; record.checkInAt = new Date().toISOString(); } save(db); send(res, 200, record); });
